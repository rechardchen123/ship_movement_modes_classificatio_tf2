...
EPOCHS = 20
batchsz = 32
log_dir = '/home/ucesxc0/Scratch/output/second_training_images/new_cnn/logs'
root = '/home/ucesxc0/Scratch/output/second_training_images/new_cnn/merged_image_for_training'
...
images, labels = load_images(root, mode='train')
db_train = tf.data.Dataset.from_tensor_slices((images, labels))
db_train = db_train.shuffle(1000).map(preprocess).batch(batchsz)
# build the validation dataset
images2, labels2 = load_images(root, mode='val')
db_val = tf.data.Dataset.from_tensor_slices((images2, labels2))
db_val = db_val.map(preprocess).batch(batchsz)
# build the test dataset
images3, labels3 = load_images(root, mode='test')
